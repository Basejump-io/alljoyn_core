...
void NameTable::RemoveUniqueName(const qcc::String& uniqueName)
{
    ...
    lock.Lock(MUTEX_CONTEXT);
    std::tr1::unordered_map<qcc::String, BusEndpoint, Hash, Equal>::iterator it = uniqueNames.find(uniqueName);
    if (it != uniqueNames.end()) {
        BusEndpoint endpoint = it->second;

        /* Remove well-known names asssociated with uniqueName */
        std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::iterator ait = aliasNames.begin();
                            ...
                            void* context)
{
    ...
    lock.Lock(MUTEX_CONTEXT);
    std::tr1::unordered_map<qcc::String, BusEndpoint, Hash, Equal>::const_iterator it = uniqueNames.find(uniqueName);
    if (it != uniqueNames.end()) {
        std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::iterator wasIt = aliasNames.find(aliasName);
                            ...
                            void* context)
{
    ...
    std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::iterator it = aliasNames.find(aliasName);
...
BusEndpoint NameTable::FindEndpoint(const qcc::String& busName) const
{
    ...
    lock.Lock(MUTEX_CONTEXT);
    if (busName[0] == ':') {
        std::tr1::unordered_map<qcc::String, BusEndpoint, Hash, Equal>::const_iterator it = uniqueNames.find(busName);
        if (it != uniqueNames.end()) {
            ep = it->second;
        }
    } else {
        std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::const_iterator it = aliasNames.find(busName);
        if (it != aliasNames.end()) {
...
void NameTable::GetBusNames(vector<qcc::String>& names) const
{
    ...
    std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::const_iterator it = aliasNames.begin();
    while (it != aliasNames.end()) {
        names.push_back(it->first);
        ++it;
    }
    std::tr1::unordered_map<qcc::String, BusEndpoint, Hash, Equal>::const_iterator uit = uniqueNames.begin();
...
void NameTable::GetUniqueNamesAndAliases(vector<pair<qcc::String, vector<qcc::String> > >& names) const
{
    ...
    std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::const_iterator ait = aliasNames.begin();
    while (ait != aliasNames.end()) {
        if (!ait->second.empty()) {
...
{
    std::tr1::unordered_map<qcc::String, deque<NameQueueEntry>, Hash, Equal>::iterator ait = aliasNames.find(busName.c_str());
