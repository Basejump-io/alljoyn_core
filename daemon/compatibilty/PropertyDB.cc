...
class _PropertyMap {
  ...
  private:
    std::std::tr1::unordered_map<qcc::StringMapKey, qcc::String> m_properties;
};
...
{
    std::std::tr1::unordered_map<qcc::StringMapKey, qcc::String>::const_iterator i = m_properties.find(name);
...
{
    for (std::std::tr1::unordered_map<qcc::StringMapKey, _PropertyMap*>::iterator i = m_modules.begin(); i != m_modules.end(); ++i) {
        delete i->second;
    }
}
...
{
    std::std::tr1::unordered_map<qcc::StringMapKey, _PropertyMap*>::const_iterator i = m_modules.find(module);
    if (i == m_modules.end()) {
